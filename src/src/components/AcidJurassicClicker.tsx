...
const randomSessionId = () => Math.random().toString(36).slice(2, 9).toUpperCase();
...
export default function AcidJurassicClicker() {
  ...
  function handleClick(label: string) {
    ...
    if (!enabled) return setStatus("Enable controls first!");
    setSelected(label);
    ...
    <div className="max-w-5xl mx-auto p-6 bg-gray-800 rounded-2xl shadow-2xl border border-emerald-700/40">
      ...
      {selected && (
        <div className="mt-6 text-center text-sm">
          <p>Last Triggered: <span className="font-semibold">{selected}</span></p>
        </div>
      )}
    </div>
  );
